...
data |> 
    ...
    ggplot(aes(ymax = ymax, ymin = ymin, xmax = 3.3, xmin = 2, fill = consentement)) +
      ...
      labs(title = "Accepteriez-vous un entretien pour approfondir cette \nenquête ?",
           subtitle = "Statistique calculée sur les réponses, vides ou remplies, à la question du prénom") +
      xlim(c(0, 4)) +   
      theme_void() +
      theme(legend.position = "right",
            text = element_text(family = "Monsterrat", size = 12),
...
```
Cette matrice chiffre la corrélation existante entre les différentes parties du cycle de vie de la donnée. Bornée entre -1 et 1, plus elle se rapproche de 1 et plus la corrélation est forte. 

Les phases de dépôt et de préparation au dépôt sont corrélées à 71%, ce sont les 2 phases les plus corrélées du cycle. Celles-ci sont aussi corrélées à la phase d'ouverture ou de partage, à respectivement 58% et 53%, elle-même liée à la réutilisation à 47%. Les parties du cycle de vie les plus liées entre elles sont donc du dépôt à l'ouverture des données.
...
planification <- data |> 
    ...
    arrange(desc(percent)) |> 
    mutate(rowname = fct_reorder(rowname, V1),
           cycle = "Planification, plan de gestion de données")
depot <- data |> 
...
reutilisation <- data |> 
    ...
    mutate(rowname = fct_reorder(rowname, V1),
           cycle = "Réutilisation")
cycle_lieux <- rbind(analystes, planification, reutilisation, prepa_depot, depot, planification, ouverture) |> 
    group_by(cycle) |> mutate(percent = round(V1/sum(V1)*100, 0)) %>% ungroup()

cycle_lieux |> 
  ggplot(aes(y = V1)) +
  geom_col(aes(x = rowname, color = rowname, fill = rowname), position = position_dodge(0.8), width = 0.7) +
  ...
  geom_text(aes(y = V1 + 2, x = rowname, label = paste(percent, "%", sep = "")), 
            color = "#333333", size = 3, check_overlap = T) +
  ...
  theme_classic() +
  facet_wrap(~cycle, strip.position="bottom") +
  theme(legend.position = "none",
        text = element_text(family = "Montserrat", size = 12),
