...
table |> 
    ggplot(aes(x = rowname, y = V1)) +
      ...
      labs(x = "", y = "Nombre de réponses", 
           title = stringr::str_wrap("Confirmation de l'ancrage 'Laboratoire' dans la question des données de la recherche"), 
           tag = "Fig. 4", 
           caption = stringr::str_wrap(paste("Question posée dans le questionnaire : Où réalisez-vous votre activité en lien avec les données de recherche ?", "-", sum(table$V1), "réponses"), width = 60)) +
      scale_x_discrete(labels = function(x) stringr::str_wrap(x, width = 40)) + 
      theme_classic() +
      custom_theme() +
      theme(legend.position = "none",
...
Remarque : non relié à la figure 5 car pas de regroupement des disciplines. 

Deux décimales pas très lisible. 
...
table <- data |> 
    ...
    t()|> as.data.frame() |> 
    rownames_to_column() |> 
    mutate(percent = round((V1 / sum(V1))*100, 2),
           rowname = str_extract(rowname, "(?<=\\[).*"),
           rowname = substr(rowname, 1, nchar(rowname)-1),
           rowname = str_extract(rowname, "^[^(]+")) |> 
    select(rowname, V1, percent) |> ungroup() |> 
...
table |> 
    ggplot(aes(x = rowname, y = V1)) +
      geom_bar(stat = "identity", width = .6, fill = "#5770BE") +
      coord_flip() +
      labs(x = "", y = "Nombre de réponses", tag = "Fig. 10", 
           title = stringr::str_wrap("Données avec lesquelles les répondants travaillent", width = 50),
         subtitle = stringr::str_wrap("", width = 50),
         caption = stringr::str_wrap("Question posée dans le questionnaire : Sur quels types de données travaillez-vous ? - ", sum(table$V1), "réponses pour 1565 répondants", width = 60)) +
      scale_x_discrete(labels = function(x) stringr::str_wrap(x, width = 40)) + 
      theme_classic() +
      custom_theme() +
      theme(legend.position = "none",
