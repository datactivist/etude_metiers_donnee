...
data |> 
    ...
    ggplot(aes(ymax = ymax, ymin = ymin, xmax = 3.3, xmin = 2, fill = consentement)) +
      ...
      labs(title = "Accepteriez-vous un entretien pour approfondir cette \nenquête ?",
           subtitle = "Statistique calculée sur les réponses") +
      xlim(c(0, 4)) +   
      theme_void() +
      theme(legend.position = "right",
            text = element_text(family = "Monsterrat", size = 12),
...
corrplot(matrix, method="color", col=col(200),  
         ...
         diag = TRUE,
         title = "Corrélation entre les différentes parties du cycle de vie de la donnée\n",
         par = c(0, 0, 5, 0))
...
set.seed(123)
res.km <- kmeans(cycle_de_vie, 3, nstart = 25)
...
# Table
table <- data |> 
    mutate(is_analyst = case_when(`Sur quelle(s) partie(s) du cycle de vie des données intervenez-vous ? [Analyse, traitement, calcul]` == "Oui" ~ "Analyse, traitement, calcul",
    ...
    group_by(is_analyst, `Sur quel domaine thématique réalisez-vous votre activité sur les données ?`) |> 
    summarise(n = n()) |> rename(col = 2) |> 
    ungroup() |> 
    mutate(col = fct_reorder(col, n),
           percent = round((n / sum(n))*100, 0),
           is_analyst = factor(is_analyst, levels = c("Autres parties du cycle", "Analyse, traitement, calcul")))

# Graphique croisé
table %>% 
  ggplot(aes(x = col, y = n, fill = is_analyst, group = is_analyst))+
    geom_bar(stat="identity", position = "dodge", width=.9, col = "white", size = 2) +
    coord_flip() +
    labs(x = "", y = "Nombre de répondants", title = "Sur quel domaine thématique réalisez-vous votre activité sur les données ?", 
