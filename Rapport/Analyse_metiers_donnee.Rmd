...
formation_activites |> 
    ...
    mutate(rowname = str_replace_all(rowname, c("données haut débit" = "HD",
                                                                   "de simulation" = "\nde simulation")),
    ...
    ggplot(aes(y = V1)) + 
     #text = paste("Besoin de formation :", formation, "\nTravail des données :", rowname, "\n", V1, "réponses soit", percent, "%"))) +
      ...
      theme_classic() +
      scale_x_discrete(labels = function(x) stringr::str_wrap(x, width = 70)) + #axis-text trop longs sur plusieurs lignes
      coord_flip() +
      theme(legend.position = "none", 
            legend.text = element_text(size = 8),
            text = element_text(family = "Montserrat", size = 12),
...
```{r prop4}
# Domaine thématique / toutes les autres
```
...
```{r prop7}
# Catégorie d'empploi / besoins de formation
```
