...
cycle_lieux <- rbind(analyse, planification, reutilisation, prepa_depot, depot, ouverture, collecte) |> 
    ...
    mutate(cycle = factor(cycle, levels = c("Collecte, création, stockage",
                                            ...
                                            "Réutilisation",
                                            "Planification, PGD")),
           col_var = case_when(cycle == "Planification, PGD" & rowname == "Infrastructure de recherche" ~ "red",
                             cycle == "Planification, PGD" & rowname == "Service central d'appui " ~ "blue",
                             .default = "grey"))
...
cycle_lieux |> 
  ...
  geom_col(aes(x = cycle, color = rowname, fill = rowname), position = "stack", width = 0.7) +
  scale_color_manual(values = c("white", "white", "white", "white", "white", "white")) +
  scale_fill_manual(values = as.character(cycle_lieux$col_var)) +
  #scale_fill_manual(values = color) +#, 
  # scale_fill_manual(values = ifelse(cycle == "Planification, PGD", 
  #                                   c('#e2c44d','#5f4496', "#ffe725", "#169B62", "#5770BE"), 
  #                                   c('#161307', '#09060f', '#191703', '#020f09', '#080b13'))) +
                                    #c('#716226', '#2f224b', '#7f7312', '#0b4d31', '#2b385f'))) +
  labs(x = "Cycle de vie de la donnée", y = "Nombre de réponses", tag = "Fig. 48",
       title = "Lieux d'activités selon l'intervention \ndans le cycle de vie de la donnée", 
       subtitle = paste0(sum(cycle_lieux$V1), " réponses"),
       caption = "") +
  custom_theme() +
  theme_classic() +
