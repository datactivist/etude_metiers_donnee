...
cycle_lieux |> 
  ggplot(aes(y = V1)) +
  geom_col(aes(x = cycle, fill = rowname), color = "white", position = "stack", width = 0.7) +
  scale_fill_manual(values = ifelse(cycle_lieux$cycle == "Planification, PGD",  
                                    c("#e2c44d", "#5f4496", "#ffe725", "#169B62", "#5770BE"),
                                    c("#716226", "#2f224b", '#7f7312', "#0b4d31", "#2b385f"))) +
  labs(x = "Cycle de vie de la donnée", y = "Nombre de réponses", tag = "Fig. 48",
       title = "Lieu d'activité autour du cycle de vie des données", 
       subtitle = "Les services d'appui mieux représentés dans la planification et l'ouverture",
       caption = "Questions posées dans le questionnaire : Sur quelle(s) partie(s) du cycle de vie des données intervenez-vous ?, Où réalisez-vous votre activité en lien avec les données de recherche ? - ", sum(cycle_lieux$V1), " réponses") +
  custom_theme() +
  theme_classic() +
  theme(legend.position = "right",
        legend.text = element_text(size = 8),
...
Demander photo à Candice pour le prochain, titre non adéquat.
...
table |> 
  ggplot(aes(x = col, y = n, fill = is_cycle, group = is_cycle))+
    ...
    labs(x = "", y = "Nombre de répondants", tag = "Fig. 43", 
         title = stringr::str_wrap("Prise en charge de l'activité 'collecte..' prédominance des BAP E", width = 50),
